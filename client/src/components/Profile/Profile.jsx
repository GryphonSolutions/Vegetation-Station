...
const Profile = ({ navigation }) => {
  const { activeUser, selectedUser, offers } = useSelector(
    (state) => state.data,
  );
  ...
  const renderRow = (index, item1, item2, item3) => {
    return (
      <View key={`View ${index}`} style={styles.row}>
        {item1 && (
          <Image key={index} style={styles.col} source={{ uri: plant }} />
        )}
        {item2 && (
          <Image key={index + 1} style={styles.col} source={{ uri: plant }} />
        )}
        {item3 && (
          <Image key={index + 2} style={styles.col} source={{ uri: plant }} />
        )}
      </View>
    );
  };
